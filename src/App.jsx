...
const generateInterviewPrep = async (resume, jobDesc) => {
  ...
  if (data.error) throw new Error(data.error); // Catch AI Gatekeeper errors
...
const InputCard = ({ title, icon: Icon, placeholder, value, onChange }) => (
  <div className="group relative h-full flex flex-col">
    <div className={`absolute -inset-0.5 bg-gradient-to-r from-pink-300 to-blue-300 rounded-2xl opacity-30 group-hover:opacity-60 transition duration-500 blur`}></div>
    <div className="relative bg-white/80 backdrop-blur-sm rounded-xl p-6 flex-1 flex flex-col shadow-sm border border-slate-100 transition-transform duration-300 group-hover:-translate-y-1">
      <div className="flex items-center gap-3 mb-4">
        <div className={`p-2.5 rounded-xl bg-gradient-to-br from-pink-300 to-blue-300 bg-opacity-10 text-slate-800 shadow-sm`}>
...
// --- VIEW COMPONENTS (Populated with Info) ---
       ...
       <div className="space-y-8">
          <div className="flex gap-6 items-start">
             ...
             <div>
                ...
                <p className="text-slate-600">Use the "Generate Interview Plan" button. This runs a deep analysis of your fit for the role.</p>
             </div>
          </div>
          <div className="flex gap-6 items-start">
             ...
             <div>
                ...
                <p className="text-slate-600">Review the "Company Intelligence" card to understand the hiring manager's pain points.</p>
             </div>
          </div>
          <div className="flex gap-6 items-start">
             <div className="w-10 h-10 rounded-full bg-slate-900 text-white flex items-center justify-center font-bold shrink-0">3</div>
             <div>
                <h4 className="text-lg font-bold text-slate-900 mb-1">Practice Live</h4>
                <p className="text-slate-600">Click "Start Practice Session" to enter the simulation. Use the microphone button to hear questions.</p>
             </div>
          </div>
       </div>
    </div>
...
const ResumeInfoView = () => (
  <div className="max-w-4xl mx-auto py-12 px-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
    <div className="text-center mb-16">
      ...
      <p className="text-lg text-slate-600 max-w-2xl mx-auto">We rewrite your resume to match the job description using high-impact language.</p>
    </div>
    <div className="grid md:grid-cols-2 gap-8 mb-16">
       <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm">
          ...
          <ul className="space-y-4">
            ...
            <li className="flex gap-3 text-slate-600"><Check size={20} className="text-emerald-500 shrink-0 mt-0.5"/> <strong>Impact Verbs:</strong> Passive phrases are replaced with "Engineered", "Led".</li>
            <li className="flex gap-3 text-slate-600"><Check size={20} className="text-emerald-500 shrink-0 mt-0.5"/> <strong>Structure:</strong> We reorder sections to match recruiter reading patterns.</li>
          </ul>
       </div>
       <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm">
...
const RadarChart = ({ data }) => {
  ...
  const dataPoints = validData.map((d, i) => { const angle = (Math.PI / 2) + (i * (Math.PI * 2) / validData.length) - Math.PI; const r = (d.score / 100) * radius; const x = center + r * Math.cos(angle); const y = center + r * Math.sin(angle); return `${x},${y}`; }).join(' ');
  return (
    <div className="relative w-full max-w-[300px] aspect-square mx-auto">
      <svg width="100%" height="100%" viewBox={`0 0 ${size} ${size}`} className="overflow-visible">
        {webPoints.map((points, i) => (<polygon key={i} points={points} fill="none" stroke="#e2e8f0" strokeWidth="1" />))}
        {validData.map((_, i) => { const angle = (Math.PI / 2) + (i * (Math.PI * 2) / validData.length) - Math.PI; const endX = center + radius * Math.cos(angle); const endY = center + radius * Math.sin(angle); return <line key={i} x1={center} y1={center} x2={endX} y2={endY} stroke="#e2e8f0" strokeWidth="1" />; })}
        <polygon points={dataPoints} fill="rgba(56, 189, 248, 0.2)" stroke="#0ea5e9" strokeWidth="2" className="drop-shadow-sm" />
        {validData.map((d, i) => { const angle = (Math.PI / 2) + (i * (Math.PI * 2) / validData.length) - Math.PI; const r = (d.score / 100) * radius; const x = center + r * Math.cos(angle); const y = center + r * Math.sin(angle); return (<g key={i} className="group cursor-pointer"><circle cx={x} cy={y} r="4" fill="#0ea5e9" stroke="white" strokeWidth="2" className="group-hover:r-6 transition-all" /><text x={center + (radius + 20) * Math.cos(angle)} y={center + (radius + 20) * Math.sin(angle)} textAnchor="middle" dominantBaseline="middle" className="text-[10px] font-bold fill-slate-500 uppercase tracking-wider">{d.label}</text></g>); })}
      </svg>
    </div>
  );
};
...
const CompanyIntelCard = ({ intel }) => {
  ...
  return (
    <div className="bg-slate-900 text-slate-100 rounded-3xl p-6 md:p-8 shadow-xl shadow-slate-900/20">
      <div className="flex items-center gap-3 mb-6 border-b border-slate-800 pb-4"><div className="p-2 bg-slate-800 rounded-lg text-sky-400"><Building size={20} /></div><div><h3 className="text-lg font-bold text-white leading-none">{renderSafe(intel.name || "Company Intelligence")}</h3><span className="text-xs text-slate-400 font-medium uppercase tracking-wider">Reconnaissance Data</span></div></div>
...
const ResumeTailor = ({ originalResume, tailoredData }) => {
  ...
  const handleCopy = () => {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText && window.isSecureContext) {
        navigator.clipboard.writeText(text).then(() => { setCopied(true); setTimeout(() => setCopied(false), 2000); }).catch(err => { fallbackCopyTextToClipboard(text); });
    } else { fallbackCopyTextToClipboard(text); }
  };
  const fallbackCopyTextToClipboard = (text) => {
      const textArea = document.createElement("textarea"); textArea.value = text; textArea.style.top = "0"; textArea.style.left = "0"; textArea.style.position = "fixed"; document.body.appendChild(textArea); textArea.focus(); textArea.select(); try { const successful = document.execCommand('copy'); if (successful) { setCopied(true); setTimeout(() => setCopied(false), 2000); } } catch (err) {} document.body.removeChild(textArea);
  };
  const generateResumeText = (data) => {
    if (!data) return '';
    let text = `${renderSafe(data.contact?.name)}\n${renderSafe(data.contact?.details)}\n\n`;
    text += `EDUCATION\n`; (data.education || []).forEach(edu => { text += `${renderSafe(edu.line)}\n`; if(edu.details) text += `${renderSafe(edu.details)}\n`; text += `\n`; });
    text += `EXPERIENCE\n`; (data.experience || []).forEach(exp => { text += `${renderSafe(exp.header)}\n`; (exp.bullets || []).forEach(bull => text += `• ${renderSafe(bull)}\n`); text += `\n`; });
    text += `PROJECTS\n`; (data.projects || []).forEach(proj => { text += `${renderSafe(proj.header)}\n`; (proj.bullets || []).forEach(bull => text += `• ${renderSafe(bull)}\n`); text += `\n`; });
    text += `SKILLS\n`; (data.skills || []).forEach(skill => { text += `${renderSafe(skill.category)}: ${renderSafe(skill.items)}\n`; }); return text;
  };
  const handleDownload = () => { const text = generateResumeText(tailoredData); const element = document.createElement("a"); const file = new Blob([text], {type: 'text/plain'}); element.href = URL.createObjectURL(file); element.download = "Tailored_Resume_PrepFlow.txt"; document.body.appendChild(element); element.click(); };
  if (!tailoredData) return <div className="text-center p-8 text-slate-400 bg-slate-50 rounded-xl border border-slate-100 mt-8">Resume tailoring was skipped or failed.</div>;
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="bg-slate-900 text-white rounded-2xl p-8 flex flex-col md:flex-row items-center justify-between gap-6 shadow-xl shadow-slate-900/20">
        <div className="flex items-center gap-6"><div className="relative w-24 h-24 flex items-center justify-center"><svg className="transform -rotate-90 w-full h-full"><circle cx="48" cy="48" r="40" stroke="rgba(255,255,255,0.1)" strokeWidth="8" fill="transparent" /><circle cx="48" cy="48" r="40" stroke="#4ade80" strokeWidth="8" fill="transparent" strokeDasharray={251} strokeDashoffset={251 - ((tailoredData.atsScore || 85) / 100) * 251} strokeLinecap="round" className="transition-all duration-1000 ease-out" /></svg><div className="absolute flex flex-col items-center"><span className="text-2xl font-bold">{tailoredData.atsScore || 85}%</span><span className="text-[10px] uppercase font-bold tracking-wider opacity-60">ATS Score</span></div></div><div><h2 className="text-2xl font-bold mb-1">Resume Optimized</h2><p className="text-slate-400 text-sm max-w-md">Your resume has been rewritten with high-perplexity phrasing.</p></div></div>
        <div className="flex gap-3"><button onClick={handleCopy} className="flex items-center gap-2 px-5 py-3 bg-white/10 text-white rounded-full font-medium hover:bg-white/20 transition-all active:scale-95 border border-white/10">{copied ? <Check size={18} /> : <Copy size={18} />}{copied ? 'Copied' : 'Copy Text'}</button><button onClick={handleDownload} className="flex items-center gap-2 px-5 py-3 bg-white text-slate-900 rounded-full font-medium hover:bg-slate-100 transition-all active:scale-95"><Download size={18} /> Download</button></div>
      </div>
      {tailoredData.resumeTalkingPoints && tailoredData.resumeTalkingPoints.length > 0 && (<div className="bg-indigo-50 p-6 rounded-2xl border border-indigo-100"><h3 className="text-lg font-bold text-indigo-900 mb-4 flex items-center gap-2"><MessageSquare size={20}/> How to Talk About Your Experience</h3><div className="grid md:grid-cols-2 gap-4">{tailoredData.resumeTalkingPoints.map((tp, idx) => (<div key={idx} className="bg-white p-4 rounded-xl shadow-sm"><h4 className="font-bold text-slate-800 text-sm mb-2">{renderSafe(tp.role)}</h4><p className="text-sm text-slate-600 leading-relaxed">"{renderSafe(tp.script)}"</p></div>))}</div></div>)}
      <div className="bg-white rounded-sm shadow-lg border border-slate-200 overflow-hidden max-w-[850px] mx-auto text-slate-900 font-sans">
         <div className="p-12 space-y-6">
            <div className="text-center border-b border-slate-300 pb-6"><h1 className="text-3xl font-serif font-bold mb-2">{renderSafe(tailoredData.contact?.name)}</h1><p className="text-sm text-slate-600 font-medium">{renderSafe(tailoredData.contact?.details)}</p></div>
